...
class A2ADocumentAgentClient:
    """Small JSON-RPC client that validates the peer's A2A Agent Card first."""
    ...
    async def _jsonrpc_url(self, client: httpx.AsyncClient) -> str:
        ...
        if "parse_itinerary_pdf" not in skills:
            raise RuntimeError("Document agent does not advertise parse_itinerary_pdf")
        ...
        raise RuntimeError("Document agent has no A2A JSON-RPC 1.0 interface")
                ...
                json={
                    ...
                    "params": {
                        "message": {
                            ...
                            "parts": [
                                {
                                    "raw": base64.b64encode(document_bytes).decode(
                                        "ascii"
                                    ),
        ...
        for artifact in result.get("artifacts", []):
            ...
            for part in parts:
                ...
                if isinstance(data, dict):
                    return ParseOutcome.model_validate(data)
        raise RuntimeError("A2A response has no structured parse artifact")
